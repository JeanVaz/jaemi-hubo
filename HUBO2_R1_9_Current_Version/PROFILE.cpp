...
void Motion_OnGestureBothArmPointJean(int T, char MotionStop,unsigned int MotionNo)
{
	static int time = 0;
	static int time1 = 0;
	static float ftime = 0.;
...
//	MotionNo = 47;	// user should change
	...
	if(MotionStop == 0)
	{
		...
		else if((time>=0)&&(time<=T))
		{
			...
			*/
			...
			FTN_half_1_cos( 1.0f,time,    50,100,0,0,&result1[LEB]);
			FTN_half_1_cos(-1.0f,time,    350,100,0,0,&result2[LEB]);
			res[LEB] = (float)(45.*(result1[LEB]+result2[LEB]));
			

			FTN_half_1_cos( 1.0f,time,    50,100,0,0,&result1[REB]);
			FTN_half_1_cos(-1.0f,time,    350,100,0,0,&result2[REB]);
